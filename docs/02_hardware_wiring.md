...
| **Current-Limiting Resistors** | 220 , 1/4W through-hole | 4 | One per LED; limits GPIO source current to safe level |
...
| **Pull-up Resistor** | 10 k, 1/4W through-hole | 1 | External pull-up for PB10 button to 3.3V (ensures HIGH default state) |
...
| **External Breadboard Button** | **PB10** | GPIOB | LOW (pressed = pulled to GND) | **External 10k to 3.3V** | Dual-function: normal press  `led flash`; held at reset  OTA bootloader mode |
...
1. MCU asserts TRIG pin HIGH for at least **10 s**
2. Sensor emits eight 40 kHz pulses
3. Sensor pulls ECHO pin HIGH: MCU captures timestamp using **TIM5** (1 MHz free-running, so 1 count = 1 s)
4. Sound travels to object and back; sensor pulls ECHO LOW
5. MCU captures end timestamp
6. Pulse width in s  distance: `distance_cm = pulse_us / 58`
...
Vout = 5V  (20k / (10k + 20k)) = 5V  0.667 = 3.33V  3.3V 
```

Alternatively, a **1k + 2k** divider works equally well for smaller resistor values (lower source impedance, faster edge response). The divider must be sized to switch cleanly during the echo pulse: avoid values above 100k total (slow edge due to parasitic capacitance).
...
```
I = (Vcc - Vf) / R = (3.3V - 2.0V) / 220 = 1.3V / 220  5.9mA
```
...
The 10k pull-up ensures a clean, defined HIGH level at rest. Without the pull-up, PB10 would float (undefined voltage) when the button is open, causing spurious triggers.
...
> *(Contribute one via PR! 10k and 20k resistors between ECHO and PB6.)*
...
###  Voltage Level Mixing (3.3V vs. 5V)
...
###  HC-SR04 Minimum Measurement Interval
...
###  PB10 Button at Reset = OTA Mode

If PB10 is held LOW at power-on or hardware reset, the **bootloader enters XMODEM OTA mode**. Be aware of this when wiring or debugging: accidentally shorting PB10 to GND during a reset will put the device in OTA mode rather than launching the application. The 10k pull-up must be correctly installed.
...
- TRIG pulse: 10s width: well above any slew rate concern
- PWM LEDs: 1kHz frequency: easily driven at low speed
- UART TX: 115200 baud  bit period = 8.7s: low speed sufficient
